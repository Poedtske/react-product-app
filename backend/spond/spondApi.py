    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=headers) as response:
            ...
            if response.status == 200:
                ...
                logging.info(f"Failed to fetch local events. Status: {response.status}")
                return []
...
async def delete_event_from_local(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.delete(url, json=event, headers=headers) as response:
            session.close()
            if response.status == 200:
                logging.info(f"Successfully deleted event")
                return True
            else:
                logging.info(f"Failed to delete event Status: {response.status}")
                return False
...
async def post_event_to_local(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=event, headers=headers) as response:
            session.close()
            if response.status == 200:
                logging.info(f"Successfully added event with ID {event['spondId']}")
                return True
            else:
                logging.info(f"Failed to add event with ID {event['spondId']}. Status: {response.status}")
                return False
...
async def update_event_in_local(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.put(url, json=event, headers=headers) as response:
            session.close()
            if response.status == 200:
                logging.info(f"Successfully updated event with ID {event['spondId']}")    
                return True           
            else:
                logging.info(f"Failed to update event with ID {event['spondId']}. Status: {response.status}")
                return False
...
async def compare_and_update_events(spond_events, local_events):
    ...
    for local_event in local_events:
        if 'spondId' in local_event:
            if local_event['spondId'] not in {event['spondId'] for event in spond_events}:
                logging.info(f"Event {local_event['spondId']} is no longer in Spond, deleting locally.")                
    ...
    for spond_event in spond_events:
        ...
        if not local_event:
            logging.info(f"Event {spond_event['spondId']} does not exist locally, adding it.")
            boolean = await post_event_to_local(spond_event)
            if boolean:
                local_events.append(spond_event)
        elif local_event != spond_event:
            logging.info(f"Event {spond_event['spondId']} differs locally, updating it.")
            logging.info(local_event)
            logging.info("----------------------------------------------")
            logging.info(spond_event)
...
async def main():
    sleep(60)
    logging.info("Fetching Spond events and comparing with local events...")
    local_events = []
    firsttime = True
    while True:
        try:
